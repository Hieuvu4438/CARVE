...
def load_train_rows(prop_seed):
    rows = []
    for k in range(K_FOLDS):
        rows += C.load(str(ROOT / "data" / "cands" / f"oof_k{k}_s{prop_seed}.jsonl"))
    return C.pool_rows([rows])


def load_eval_rows(split, prop_seed):
    return C.pool_rows([C.load(str(ROOT / "data" / "cands" / f"{split}_s{prop_seed}.jsonl"))])
...
def main(a):
    ...
    if a.smoke:   # code-path test only: train on a few dev windows, numbers meaningless
        tr_rows = load_eval_rows("dev", a.prop_seed)[:60]
        train_w = dev_w
    else:
        tr_rows = load_train_rows(a.prop_seed)
    for r in tr_rows:
    ...
    model = Verifier(a.model_name, len(tok), dropout=a.dropout).to(device)
...
if __name__ == "__main__":
    ...
    a = ap.parse_args()
    a.name = a.name or f"verifier_s{a.seed}"
    main(a)
