"""Stage 3: turn verifier probabilities into final predictions.

For every candidate the verifier returns p over {reject, role_1..role_9}.

    keep score  = 1 - p(reject)
    role        = argmax of an alpha-mix of the verifier's role distribution and
                  the proposer's normalised role mass (`mix_roles`)
    event type  = argmax_t log p(t) + lambda * sum_i log P(role_i | t)   (`joint_type`)

A window's spans are chosen greedily: candidates with keep score >= theta and
length >= min_len are visited in decreasing keep score and accepted unless they
collide with an accepted span. Collision rules searched on dev:
    ...
    "wis"  exact maximum-weight non-overlapping set (weighted interval scheduling)

The five rule parameters (theta, min_len, collision rule, alpha, lambda) are
chosen on dev only and frozen before test (scripts/freeze_and_test.py). Window
AAO spans and the trigger (used only for ROUGE-L) come from the proposer.
"""
...
def mix_roles(rows, probs_per_window, alpha):
    ...
    Motivation (measured on dev during development, original CARVE proposer):
    the verifier ranks keep/drop better than the proposer (AUC 0.890 vs 0.860)
    but assigns roles worse (77.1 % vs 81.3 % on matched candidates), so the two
    sources of role evidence are complementary. alpha is chosen on dev.
    """
    if alpha >= 1.0:
