"""Stage 2: the HONE verifier.

For one candidate span the verifier reads
...
small feature vector, and outputs one of 10 classes: `reject`, or one of the 9
scored roles. It therefore decides keep/drop from the *content* of the span in
context rather than from a mean token posterior, and can re-assign the role.

Training candidates are out-of-fold (see scripts/propose.py), so the verifier
learns from the proposer's real mistakes on windows the proposer never saw.
"""
...
def features(c, n_words):
    """Proposer evidence for one candidate (17 values, all roughly in [0, 1]):
    role mass x9, P(O), conf, conf_max, seed agreement, log length, relative
    start, relative end, and a part count. The part count is 0 for every
    proposer span; it belonged to a candidate-composition variant that was
    tested and rejected, and is kept so the input layout matches the reported
    verifier exactly."""
    length = c["e"] - c["s"]
...
class Verifier(nn.Module):
    def __init__(self, model_name, vocab_size, n_labels=len(LABEL2ID), dropout=0.1,
                 use_feats=True):
        super().__init__()
        ...
        self.head = nn.Sequential(nn.Linear(3 * h + (fdim if use_feats else 0), h), nn.GELU(), nn.Dropout(dropout),
