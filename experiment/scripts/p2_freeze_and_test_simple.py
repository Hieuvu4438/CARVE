...
Isolated Phase-7 copy: run `... abl_single_head freeze`, timestamp and register
the rule via experiment/scripts/p2_protocol.py, then run `... abl_single_head test`.
"""
...
if PREFIX != "abl_single_head":
    raise ValueError("the isolated experiment copy is registered only for abl_single_head")
if MODE not in {"freeze", "test"}:
    raise ValueError("the isolated experiment copy requires freeze or test mode")
...
rule_path = os.path.join(HERE, "assets", f"decoding_rules.{PREFIX}.json")
if MODE == "freeze":
    ...
    json.dump({"rules": {"tau": {"_": best["tau"]}, "merge_gap": 0, "min_len": best["min_len"]},
              ...
              open(rule_path, "w"), indent=2)
else:
    frozen = json.load(open(rule_path))
    if not frozen.get("frozen_at"):
        raise ValueError("test requires a timestamped frozen rule")
    test_log = open(os.path.join(HERE, "..", "TEST_LOG.md")).read()
    registered = [line for line in test_log.splitlines()
                  if "| T-P2b |" in line and "| RUNNING |" in line]
    if len(registered) != 1 or rule_path not in registered[0]:
        raise ValueError("test requires the T-P2b registration in TEST_LOG")
    best = {"tau": frozen["rules"]["tau"]["_"], "min_len": frozen["rules"]["min_len"]}
...
if MODE == "test":
    ...
    res = []
    for s in SEEDS:
        out = os.path.join(HERE, "preds", f"test_{PREFIX}_s{s}.jsonl")
