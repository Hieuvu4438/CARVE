...
from clave.paths import default_data_dir
...
def write_candidates(ckpt, windows, out, seed):
    ...
    C.save(str(temp), rows_for(windows, span_p, type_p, seed))
    if len(C.load(str(temp))) != len(windows):
        raise ValueError(f"candidate row count mismatch: {temp}")
    temp.replace(out)
...
def mode_eval(seed):
    ck = run_dir(f"proposer_s{seed}") / "best.pt"
    assert ck.exists(), f"missing proposer checkpoint {ck} (train it with clave.train first)"
    for split in ["dev", "test"]:
        print(f"[eval] {split}: ", end="")
        out = candidate_path(f"{split}_s{seed}.jsonl", write=True)
        if out.exists():
            expected = len(load_split(split))
            if len(C.load(str(out))) != expected:
                raise ValueError(f"existing candidate file is incomplete: {out}")
            print(f"{out} already exists ({expected} rows), skipping")
            continue
        write_candidates(ck, load_split(split), out, seed)


def mode_insample(seed):
    ck = run_dir(f"proposer_s{seed}") / "best.pt"
    out = candidate_path(f"insample_s{seed}.jsonl", write=True)
    if out.exists():
        if len(C.load(str(out))) != len(load_split("train")):
            raise ValueError(f"existing in-sample candidate file is incomplete: {out}")
        print(f"[insample] {out} already exists (1,278 rows), skipping")
        return
    write_candidates(ck, load_split("train"), out, seed)
...
def mode_oof(fold, seed, epochs=None):
    ...
    if out.exists():
        expected = len(load_split("heldout", str(fold_dir)))
        if len(C.load(str(out))) != expected:
            raise ValueError(f"existing OOF candidate file is incomplete: {out}")
        print(f"[oof] {out} already exists ({expected} rows), skipping")
        return
    from clave.train import load_config, run
    run_name = f"oof_k{fold}_s{seed}"
    cfg = load_config(SOURCE_ROOT / "configs" / "proposer.json")
    ...
    ck = run_dir(run_name, write=True) / "best.pt"
